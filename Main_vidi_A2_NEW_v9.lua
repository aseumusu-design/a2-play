...
do -- Emote Tab
    ...
    emoteSection:AddButton({
        ...
        Callback = function()
            pcall(function()
                loadstring(game:HttpGetAsync("https://pastefy.app/wa3v2Vgm/raw"))()
            end)
        end
    })
end
...
-- =====================================================
-- BYPASS COOLDOWN (Jeff / The Killer) - Hook tombol move1 + CooldownEvent
-- Skill hanya aktif/nonaktif saat tombol move1 asli game ditekan
-- =====================================================
getgenv().KYS_JeffMove1Connection = nil
getgenv().KYS_JeffCooldownConnection = nil
getgenv().KYS_JeffSkillActive = false

function KYS_StartJeffCooldownBypass()
    -- Bersihkan koneksi lama biar tidak numpuk
    if getgenv().KYS_JeffMove1Connection then
        getgenv().KYS_JeffMove1Connection:Disconnect()
        getgenv().KYS_JeffMove1Connection = nil
    end
    if getgenv().KYS_JeffCooldownConnection then
        getgenv().KYS_JeffCooldownConnection:Disconnect()
        getgenv().KYS_JeffCooldownConnection = nil
    end
    ...
    local killerRemotes = ReplicatedStorage:FindFirstChild("Remotes")
        and ReplicatedStorage.Remotes:FindFirstChild("Killers")
        and ReplicatedStorage.Remotes.Killers:FindFirstChild("Killer")

    local activatePowerRemote = killerRemotes and killerRemotes:FindFirstChild("ActivatePower")
    local deactivateRemote = killerRemotes and killerRemotes:FindFirstChild("Deactivatefromclient")
    local cooldownEventRemote = killerRemotes and killerRemotes:FindFirstChild("CooldownEvent")

    -- Simpan remotes agar bisa dipakai fungsi Stop
    getgenv().KYS_JeffRemotes = {
        Activate = activatePowerRemote,
        Deactivate = deactivateRemote,
        Cooldown = cooldownEventRemote,
    }

    -- Sadap klik tombol asli game (move1)
    pcall(function()
        local playerGui = player:FindFirstChild("PlayerGui")
        local killerMob = playerGui and playerGui:FindFirstChild("Killer-mob")
        local controls = killerMob and killerMob:FindFirstChild("Controls")
        local move1Button = controls and controls:FindFirstChild("move1")

        if move1Button then
            getgenv().KYS_JeffMove1Connection = move1Button.MouseButton1Click:Connect(function()
                if not VD.KILLER_InfFrenzy then return end
                getgenv().KYS_JeffSkillActive = not getgenv().KYS_JeffSkillActive

                local char = player.Character
                if getgenv().KYS_JeffSkillActive then
                    -- AKTIFKAN SKILL & BYPASS CD
                    if char then
                        char:SetAttribute("Frenzy", true)
                    end
                    if activatePowerRemote then
                        pcall(function()
                            activatePowerRemote:FireServer(true)
                        end)
                    end
                else
                    -- MATIKAN SKILL SECARA NORMAL (Biar tidak nyangkut)
                    if char then
    ...
    -- Reset cooldown instan di sisi client
    if cooldownEventRemote then
        getgenv().KYS_JeffCooldownConnection = cooldownEventRemote.OnClientEvent:Connect(function()
            if not VD.KILLER_InfFrenzy then return end
            pcall(function()
                cooldownEventRemote:FireServer(0)
            end)
        end)
    end
end
...
function KYS_StopJeffCooldownBypass()
    -- Putuskan koneksi hook
    if getgenv().KYS_JeffMove1Connection then
        pcall(function() getgenv().KYS_JeffMove1Connection:Disconnect() end)
        getgenv().KYS_JeffMove1Connection = nil
    end
    if getgenv().KYS_JeffCooldownConnection then
        pcall(function() getgenv().KYS_JeffCooldownConnection:Disconnect() end)
        getgenv().KYS_JeffCooldownConnection = nil
    end

    -- Matikan skill secara normal kalau masih aktif
    pcall(function()
        local player = game:GetService("Players").LocalPlayer
        local char = player.Character
        getgenv().KYS_JeffSkillActive = false
        if char and char:GetAttribute("Frenzy") == true then
            char:SetAttribute("Frenzy", false)
            local remotes = getgenv().KYS_JeffRemotes
            if remotes and remotes.Deactivate then
                remotes.Deactivate:FireServer()
            end
        end
    end)
end
